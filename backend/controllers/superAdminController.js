...
export const getPaymentProofDetails = catchAsyncErrors(
  async (req, res, next) => {
    const { id } = req.params;
...
export const updateProofStatus = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!proof) {
    return next(ErrorHandler("Payment proof not found", 404));
  }
...
export const fetchAllUsers = catchAsyncErrors(async (req, res, next) => {
  ...
  const transfromDataToMonthlyArray = (data, totalMonths = 12) => {
  ...
  const bidderArray = transfromDataToMonthlyArray(bidders);
  const auctioneersArray = transfromDataToMonthlyArray(auctioneers);
